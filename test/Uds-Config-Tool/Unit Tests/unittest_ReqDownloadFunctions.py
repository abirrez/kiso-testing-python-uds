...
class RequestDownloadTestCase(unittest.TestCase):
    ...
    def test_req_download_request(self, canTp_send, canTp_recv):
        ...
        # ... creates the uds object and returns it; also parses out the rdbi info and attaches the __request_download to requestDownload in the uds object, so can now call below

        b = a.requestDownload(
            FormatIdentifier = [0x00],
            MemoryAddress = [0x40, 0x03, 0xE0, 0x00],
            MemorySize = [0x00, 0x00, 0x0E, 0x56],
        )  # ... calls __request_download, which does the Uds.send
    ...
    @mock.patch("uds.TestTp.recv")
    @mock.patch("uds.TestTp.send")
    def test_req_download_request02(self, canTp_send, canTp_recv):
    ...
    @mock.patch("uds.TestTp.recv")
    @mock.patch("uds.TestTp.send")
    def test_req_download_negative_request_0x13(self, canTp_send, canTp_recv):
    ...
    @mock.patch("uds.TestTp.recv")
    @mock.patch("uds.TestTp.send")
    def test_wdbi_neg_response_0x22(self, canTp_send, canTp_recv):
    ...
    @mock.patch("uds.TestTp.recv")
    @mock.patch("uds.TestTp.send")
    def test_wdbi_neg_response_0x72(self, canTp_send, canTp_recv):
