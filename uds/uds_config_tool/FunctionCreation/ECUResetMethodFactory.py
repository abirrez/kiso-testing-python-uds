...
check_function_template = str(
    ...
    "    reset_type = DecodeFunctions.build_int_from_list(input[{5}:{6}])\n"
    ...
    '    if(reset_type != resetTypeExpected): raise Exception("Reset Type Received not as expected. Expected: {{0}}; Got {{1}}".format(resetTypeExpected, reset_type))'
)
    ...
    def create_request_function(diag_service_element, xml_elements):
        ...
        for param in params_element:
            ...
            if semantic == "SERVICE-ID":
                ...
                reset_type = [int(param.find("CODED-VALUE").text)]
                if reset_type[0] >= SUPPRESS_RESPONSE_BIT:
                    pass
        ...
        func_string = request_func_template.format(
            short_name, service_id, reset_type, SUPPRESS_RESPONSE_BIT
        )
        exec(func_string)
        ...
        for param in params_element:
            try:
                ...
                if semantic == "SERVICE-ID":
                    ...
                    list_length = int(bit_length / 8)
                    reset_type_start = start_byte
                    reset_type_end = start_byte + list_length
                    total_length += list_length
                elif semantic == "DATA":
