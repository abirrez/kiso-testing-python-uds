...
request_func_template = str(
    "def {0}(option_record,suppress_response=False):\n"
    "    control_type = {2}\n"
    "    suppressBit = {6} if suppress_response else 0x00\n"
    "    control_type[0] += suppressBit\n"
    "    encoded = []\n"
    "    if option_record is not None:\n"
    "        if type(option_record) == list and type(option_record[0]) == tuple:\n"
    "            drDict = dict(option_record)\n"
    "            {4}\n"
    "{5}\n"
    "    return {1} + control_type + {3} + encoded"
)
...
check_function_template = str(
    ...
    "    control_type = DecodeFunctions.build_int_from_list(input[{6}:{7}])\n"
    "    routine_id = DecodeFunctions.build_int_from_list(input[{8}:{9}])\n"
    ...
    '    if(control_type != controlTypeExpected): raise Exception("Control Type Received not expected. Expected {{0}}; Got {{1}} ".format(controlTypeExpected, control_type))\n'
    '    if(routine_id != routineIdExpected): raise Exception("Routine Id Received not as expected. Expected: {{0}}; Got {{1}}".format(routineIdExpected, routine_id))'
)
        ...
        for param in params_element:
            try:
                ...
                if semantic == "SERVICE-ID":
                    ...
                    control_type = [int(param.find("CODED-VALUE").text)]
                    if control_type[0] >= SUPPRESS_RESPONSE_BIT:
                        pass
                        # raise ValueError("ECU Reset:reset type exceeds maximum value (received {0})".format(reset_type[0]))
                elif semantic == "ID":
                    routine_id = DecodeFunctions.int_array_to_int_array(
                    ...
                    if (encoding_type) == "A_ASCIISTRING":
                        function_string_list = "DecodeFunctions.string_to_int_list(drDict['{0}'], None)".format(
                            long_name
                        )
                        function_string_single = (
                            "DecodeFunctions.string_to_int_list(option_record, None)"
                        )
                        ...
                        function_string_single = "DecodeFunctions.int_value_to_byte_array(option_record, {0})".format(
                            bit_length
                        )
                    else:
                        function_string_list = "drDict['{0}']".format(long_name)
                        function_string_single = "option_record"
                    """
...
Also, we will most need to handle scaling at some stage within DecodeFunctions.py (for RDBI at the very least)
        ...
        func_string = request_func_template.format(
            ...
            control_type,  # 2
            routine_id,  # 3
            "\n            ".join(encode_functions),  # ... handles input via list # 4
            encode_function,  # ... handles input via single value # 5
            SUPPRESS_RESPONSE_BIT,
        )  # 6
        exec(func_string)
        return (locals()[short_name], str(control_type))
        ...
        for param in params_element:
            try:
                ...
                if semantic == "SERVICE-ID":
                    ...
                    list_length = int(bit_length / 8)
                    control_type_start = start_byte
                    control_type_end = start_byte + list_length
                    total_length += list_length
                elif semantic == "ID":
                    routine_id = int(param.find("CODED-VALUE").text)
                    bit_length = int(
                        (param.find("DIAG-CODED-TYPE")).find("BIT-LENGTH").text
                    )
                    list_length = int(bit_length / 8)
                    routine_id_start = start_byte
                    routine_id_end = start_byte + list_length
                    total_length += list_length
                elif semantic == "DATA":
                    data_object_element = xml_elements[
                        (param.find("DOP-REF")).attrib["ID-REF"]
